...
import (
	"github.com/SENERGY-Platform/converter/lib/converter/speedlevel"
	"reflect"
	"testing"
)
...
func TestSpeedlevelPercentToNumber(t *testing.T) {
	...
	for _, test := range table {
		out, err := Cast(test.Percent, speedlevel.PercentageId, speedlevel.OneToTenId)
		if err != nil {
			t.Fatal(err)
		}
		result, ok := out.(int64)
...
func TestSpeedlevelNumberToPercent(t *testing.T) {
	...
	for _, test := range table {
		out, err := Cast(test.Number, speedlevel.OneToTenId, speedlevel.PercentageId)
		if err != nil {
			t.Fatal(err)
		}
		result, ok := out.(int64)
