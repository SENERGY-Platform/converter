...
func TestSpeedlevelPercentToNumber(t *testing.T) {
	...
	for _, test := range table {
		out, err := Cast(test.Percent, characteristics.Percentage, characteristics.OneToTen)
		if err != nil {
			t.Fatal(err)
		}
		result, ok := out.(int64)
...
func TestSpeedlevelNumberToPercent(t *testing.T) {
	...
	for _, test := range table {
		out, err := Cast(test.Number, characteristics.OneToTen, characteristics.Percentage)
		if err != nil {
			t.Fatal(err)
		}
		result, ok := out.(int64)
